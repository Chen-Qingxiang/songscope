...
真实数据验证表明，同一年内只有先后关系、而不宜生成具体月日的事件需要稳定的 `sequence_index`；它只用于投影排序，不改变 `TemporalExtent` 的精度。

黄州案例还要求在第一阶段 `service_episode` 投影中区分 `service` 与 `residence`：后者可以没有官职概念，但必须有人物、地点、时间、形成事件和证据。这样“黄州团练副使”“本州安置”仍属于任命组件，而“在黄州实际居住”属于独立状态，不会被错误解释为主持黄州军政。
